/** Maximum source size eligible for optional asynchronous Shiki enhancement. */
export const MAX_HL_CHARS = 32_000;
const MAX_CACHE_ENTRIES = 48;

type CodeToAnsi = (code: string, lang: string, theme: string) => Promise<string>;
export type ShikiLoader = () => Promise<CodeToAnsi>;
...
/**
 * Small, viewer-local async cache. The synchronous Pi highlighter remains the
 * first paint; Shiki is only requested by an expanded card that has metadata.
 */
export class ShikiHighlightCache {
  private readonly cache = new Map<string, string[]>();
  private readonly pending = new Map<string, Pending>();

  constructor(private readonly loader: ShikiLoader = loadDefault) {}
  ...
  get(
    ...
    if (cached) {
      // LRU: a frequently viewed card should not evict itself immediately.
      ...
      .then((ansi) => {
        ...
        for (const subscriber of pending.subscribers) {
          ...
          try {
            ...
            // A stale card must not prevent another card from being repainted.
          }
        }
      })
      .catch(() => {
        // The synchronous Pi highlighter remains the durable fallback.
      })
