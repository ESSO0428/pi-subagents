...
// ============================================================================
// Tool converter: Nico string[] → built-in and extension selectors
// ============================================================================

/**
 * Keep Nico tool overrides aligned with the current agent-file parser:
 * wildcard entries expand built-ins, while `ext:` entries narrow extension
 * tools instead of being reported as unknown built-in tools.
 */
export function resolveNicoTools(
  tools: string[] | false,
): { builtinToolNames: string[]; extSelectors?: string[] } {
  if (tools === false) return { builtinToolNames: [], extSelectors: undefined };

  const isWildcard = (entry: string) => entry === "*" || entry.toLowerCase() === "all";
  const hasWildcard = tools.some(isWildcard);
  const builtinToolNames = tools.filter(
    (entry) => !isWildcard(entry) && !entry.startsWith("ext:"),
  );
  const extSelectors = tools.filter((entry) => entry.startsWith("ext:"));

  return {
    builtinToolNames: hasWildcard
      ? [...new Set([...BUILTIN_TOOL_NAMES, ...builtinToolNames])]
      : builtinToolNames,
...
export function applyNicoOverride(
  ...
  if (override.tools !== undefined) {
    next = { ...next, ...resolveNicoTools(override.tools) };
    modified = true;
  }
...
function createAgentFromOverride(
  ...
  const tools = override.tools === undefined
    ...
    : resolveNicoTools(override.tools);
